...
class EscrituraPaletasTests(unittest.TestCase):
    def test_gpl_ida_y_vuelta(self):
        colores = [QColor(255, 0, 0), QColor(0, 128, 255)]
        with tempfile.TemporaryDirectory() as carpeta:
            ruta = os.path.join(carpeta, "salida.gpl")
            self.assertTrue(guardar_paleta(ruta, colores, nombre="Prueba"))
            self.assertEqual(_rgb(cargar_paleta(ruta)),
                             [(255, 0, 0, 255), (0, 128, 255, 255)])

    def test_txt_hexadecimal_conserva_el_alfa(self):
        colores = [QColor(255, 0, 0, 128), QColor(0, 128, 255)]
        with tempfile.TemporaryDirectory() as carpeta:
            ruta = os.path.join(carpeta, "salida.txt")
            self.assertTrue(guardar_paleta(ruta, colores))
            self.assertEqual(_rgb(cargar_paleta(ruta)),
                             [(255, 0, 0, 128), (0, 128, 255, 255)])

    def test_paleta_vacia_o_invalida_no_escribe_nada(self):
        with tempfile.TemporaryDirectory() as carpeta:
            ruta = os.path.join(carpeta, "vacia.gpl")
            self.assertFalse(guardar_paleta(ruta, []))
...
class GestionMuestrasTests(unittest.TestCase):
    ...
    def test_guardar_y_cargar_un_conjunto_con_nombre(self):
        with tempfile.TemporaryDirectory() as carpeta:
            ...
            editor._refresh_collection_combo()
            # Elegir el conjunto en el combo lo carga directamente.
            with patch.object(editor, "_confirmar_muestras", return_value=True):
    ...
    def test_cancelar_la_carga_desde_el_combo_repone_la_seleccion(self):
        with tempfile.TemporaryDirectory() as carpeta:
            principal, editor = self._crear_editor(carpeta, "#ffff0000")
            editor._custom_collections = [{
                "name": "Otra", "colors": [QColor("#00ff00")],
            }]
            editor._refresh_collection_combo()
            ...
            self.assertEqual(_rgb(editor._custom_colors), [(255, 0, 0, 255)])
            self.assertIsNone(editor.collection_combo.currentData())
            editor.deleteLater()
            principal.deleteLater()

    def test_exportar_muestras_a_gpl(self):
        with tempfile.TemporaryDirectory() as carpeta:
            principal, editor = self._crear_editor(
                carpeta, "#ffff0000,#8000ff00")
            ruta = os.path.join(carpeta, "muestras.gpl")

            with patch("PySide6.QtWidgets.QFileDialog.getSaveFileName",
                       return_value=(ruta, "")):
                editor.export_palette()

            # GPL no lleva alfa: el verde semitransparente sale opaco.
            self.assertEqual(_rgb(cargar_paleta(ruta)),
