"""Lectura tolerante y escritura de formatos habituales de paletas de color."""
...
def _texto_gpl(colores, nombre):
    """Paleta GIMP: universal (GIMP, Krita, Inkscape...) pero SIN canal alfa."""
    lineas = ["GIMP Palette", "Name: %s" % (nombre or "Imago"), "Columns: 8", "#"]
    for color in colores:
        lineas.append("%3d %3d %3d\t%s" % (
            color.red(), color.green(), color.blue(), color.name().upper()))
    return "\n".join(lineas) + "\n"


def _texto_hex_argb(colores, nombre):
    """Lista hexadecimal AARRGGBB (estilo Paint.NET): conserva el alfa y la
    vuelve a leer _parse_lista_texto."""
    lineas = ["; %s" % (nombre or "Imago")]
    for color in colores:
        lineas.append("%08X" % color.rgba())
    return "\n".join(lineas) + "\n"


def guardar_paleta(ruta, colores, nombre=None):
    """Escribe la paleta de forma atómica según la extensión del destino:
    ``.gpl`` (GIMP, sin alfa) o lista hexadecimal AARRGGBB (``.txt``).
    Devuelve True solo si el archivo quedó publicado."""
    colores = _sin_duplicados(colores)
    if not colores:
        return False
    extension = os.path.splitext(os.fspath(ruta))[1].lower()
    if extension == ".gpl":
        contenido = _texto_gpl(colores, nombre)
    else:
        contenido = _texto_hex_argb(colores, nombre)
    datos = contenido.encode("utf-8")

    def escritor(temporal):
        with open(temporal, "wb") as archivo:
            archivo.write(datos)
        return True

    return escribir_atomico(ruta, escritor)
