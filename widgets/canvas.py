...
class Canvas(QWidget):
    ...
    def invert_mask(self):
        """Invierte la máscara activa (blanco ↔ negro) sin tocar la capa."""
        layer = self.get_active_layer_obj()
        if layer is None or not layer.has_mask():
            return False
        from models.layer_commands import InvertMaskCommand
        self.undo_stack.push(InvertMaskCommand(
            self, self.active_layer_index))
        return True
    ...
    def copy_selection(self):
        ...
        return self._copy_image_to_clipboard(layer_img, require_selection=True)

    def _copy_image_to_clipboard(self, image, require_selection=False):
        """Copia una imagen completa o recortada por la selección vigente."""
        from PySide6.QtWidgets import QApplication
        from PySide6.QtCore import QRect, QPoint

        if image is None or image.isNull():
            return False
        has_selection = self.selection is not None and not self.selection.isEmpty()
        if require_selection and not has_selection:
            return False

        if has_selection:
            bounds = self.selection.boundingRect().toAlignedRect()
            bounds = bounds.intersected(
                QRect(0, 0, self.base_width, self.base_height))
        else:
            bounds = QRect(0, 0, self.base_width, self.base_height)
        if bounds.isEmpty():
            return False
    ...
    def copy_visible(self):
        """Copia el compuesto visible, respetando la selección si existe."""
        from models.layer import visible_efectiva
        if not any(visible_efectiva(layer) for layer in self.layers):
            return False
        image = self.render_flat_image(Qt.transparent)
        return self._copy_image_to_clipboard(image)

    def new_layer_from_visible(self):
        """Crea una capa raster con el compuesto visible, sin tocar las fuentes."""
        from models.layer import visible_efectiva
        if not any(visible_efectiva(layer) for layer in self.layers):
            return False
        layer = Layer(self.base_width, self.base_height,
                      name=t("layer.from_visible"))
        layer.image = self.render_flat_image(Qt.transparent).convertToFormat(
            QImage.Format_ARGB32)
        from models.layer_commands import AddLayerCommand
        self.undo_stack.push(AddLayerCommand(
            self, layer=layer, text=t("hist.new_from_visible")))
        return True

    def selection_from_layer_alpha(self):
        """Reemplaza la selección por el alfa visible de la capa activa."""
        layer = self.get_active_layer_obj()
        if layer is None:
            return False
        from tools.numpy_utils import alpha_mask, path_from_mask
        path = path_from_mask(alpha_mask(layer.render_image()))
        if path is None or path.isEmpty():
            return False
        from tools.commands import SelectionChangeCommand
        self.undo_stack.push(SelectionChangeCommand(
            self, self.selection, path, t("hist.alpha_to_selection"),
            tool_id="alpha_selection"))
        return True
    ...
    def crop_to_content(self):
        """Recorta el documento al alfa del compuesto visible."""
        from tools.numpy_utils import alpha_bounds
        bounds = alpha_bounds(self.render_flat_image(Qt.transparent))
        if bounds is None:
            return False
        if (bounds.x() == 0 and bounds.y() == 0
                and bounds.width() == self.base_width
                and bounds.height() == self.base_height):
            return False
        from models.layer_commands import CropCommand
        self.undo_stack.push(CropCommand(
            self, bounds, text=t("hist.crop_to_content")))
        return True

    def center_active_layer(self):
        """Centra el contenido visible de la capa activa en el lienzo."""
        layer = self.get_active_layer_obj()
        if layer is None or getattr(layer, "position_locked", False):
            return False
        from tools.numpy_utils import alpha_bounds
        bounds = alpha_bounds(layer.render_image())
        if bounds is None:
            return False
        dx = (self.base_width - bounds.width()) // 2 - bounds.x()
        dy = (self.base_height - bounds.height()) // 2 - bounds.y()
        if dx == 0 and dy == 0:
            return False
        from models.layer_commands import CenterLayerCommand
        self.undo_stack.push(CenterLayerCommand(
            self, self.active_layer_index, dx, dy))
        return True
