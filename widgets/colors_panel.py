...
class ColorsPanel(QWidget):
    ...
    def sync_from_canvas(self, canvas):
        """Conecta una pestaña conservando los colores globales del panel.

        Los colores primario y secundario pertenecen a la sesión de edición, no
        al documento. Cada lienzo mantiene una copia porque las herramientas la
        consultan directamente, pero al activar una pestaña esa copia adopta el
        estado visible del panel en vez de restaurar un color antiguo suyo.
        """
        if not canvas:
            return
        canvas.color_picked_callback = self.on_color_picked
        primario = self.preview_box.color() or QColor("#000000")
        secundario = self.secondary_box.color() or QColor("#FFFFFF")
        canvas.brush_color = QColor(primario)
        canvas.brush_color_secondary = QColor(secundario)
        self._sync_mirror()
