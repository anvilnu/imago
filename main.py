...
# versión posterior. Este programa se distribuye SIN NINGUNA GARANTÍA. Consulta
# el archivo LICENSE (GNU GPL v3) para más detalles.
import sys
import os
import app_paths

# Se captura antes de que el ejecutable congelado cambie al directorio interno
# de PyInstaller. Windows suele entregar rutas absolutas al usar «Abrir con»,
# pero conservar el origen hace que una ruta relativa también se resuelva bien.
_DIRECTORIO_INVOCACION = os.getcwd()


def _rutas_inicio_desde_argumentos(argumentos, directorio_invocacion=None):
    """Normaliza los archivos recibidos al iniciar Imago desde el sistema."""
    base = os.path.abspath(directorio_invocacion or os.getcwd())
    rutas = []
    for argumento in list(argumentos)[1:]:
        if not argumento:
            continue
        ruta = os.path.expanduser(os.fspath(argumento))
        if not os.path.isabs(ruta):
            ruta = os.path.join(base, ruta)
        rutas.append(os.path.normpath(ruta))
    return rutas


def _abrir_rutas_inicio(ventana, rutas):
    """Abre en orden todos los archivos entregados por el sistema operativo."""
    for ruta in rutas:
        ventana.open_path(ruta)
...
class MainWindow(AccionesMenuIA, AccionesMenuAjustes, OpcionesHerramientas,
    ...
    def __init__(self):
        ...
        # Autoguardado y recuperación ante fallos: copia de seguridad de las
        # pestañas con cambios según el intervalo de Preferencias (3 min por
        # defecto). Si la sesión anterior no se cerró bien, se ofrece recuperar.
        from models.autosave import AutoSaveManager, intervalo_desde_settings
        intervalo_autoguardado = intervalo_desde_settings(self.settings)
        self.autosave = AutoSaveManager(
            self, interval_min=intervalo_autoguardado)
        self.autosave.start()
    ...
    def update_layer_menu_state(self):
        ...
        for key in ("new", "new_visible", "duplicate", "group", "toggle_vis",
                    "copy_visible", "alpha_selection", "crop_content",
                    "flip_h", "flip_v", "rot_cw", "rot_ccw", "rot_180",
                    "properties", "fx_menu", "blend_menu"):
            if key in acts:
        ...
        if "center" in acts:
            acts["center"].setEnabled(
                not bool(getattr(canvas.layers[idx], "position_locked", False)))
        ...
        if "mask_create" in acts:
            ...
            acts["mask_invert"].setEnabled(has_mask)
    ...
    def on_tab_changed(self, index):
        ...
        diagnostico = getattr(self, "_document_diagnostics_dialog", None)
...
if __name__ == "__main__":
    import sys, os
    _rutas_inicio = _rutas_inicio_desde_argumentos(
        sys.argv, _DIRECTORIO_INVOCACION)
    ...
    window.show()
    if _rutas_inicio:
        # El primer procesamiento se difiere hasta que la ventana sea visible;
        # así los avisos y preguntas de formatos animados tienen padre válido.
        QTimer.singleShot(0, lambda: _abrir_rutas_inicio(window, _rutas_inicio))
    sys.exit(app.exec())
