...
def _imagen_miniatura_capa(layer):
    """Contenido que representa la miniatura principal de una capa.

    Si existe una máscara, esta ya tiene su propia miniatura al lado. Mostrar
    también su resultado sobre la miniatura principal hacía parecer que los
    píxeles originales desaparecían al pintar de negro en la máscara. En ese
    caso se muestra el contenido anterior a la máscara; sin máscara se conserva
    la vista con efectos que ya ofrecía el panel.
    """
    if layer.has_mask():
        return layer.render_sin_mascara()
    return layer.render_with_effects()
...
class LayersPanel(QWidget):
    THUMB_REFRESH_MS = 250

    def __init__(self, canvas):
        super().__init__()
        self.canvas = canvas
        self._thumb_canvas_conectado = None
        ...
        #   layer_merge.png, layer_flatten.png, layer_mask.png,
        ...
        self.btn_flatten = self._make_btn(
            ":/icons/layer_flatten.png", "▤", t("menu.layers.flatten"), self.flatten)
        self.btn_mask = self._make_btn(
            ":/icons/layer_mask.png", "◩", t("layer.mask.add"), self.add_mask)
        ...
        for btn in (self.btn_add, self.btn_remove, self.btn_duplicate,
                    self.btn_group, self.btn_merge, self.btn_flatten, self.btn_mask,
                    self.btn_properties):
            btn_layout.addWidget(btn)

        layout.addLayout(btn_layout)

        # Refresco en vivo de las miniaturas, dirigido por los cambios visuales
        # del lienzo. El single-shot agrupa las ráfagas de un trazo; en reposo no
        # existe sondeo. Las firmas por fila evitan regenerar las demás capas.
        self._thumb_rows = []   # [(layer, QLabel)] de las filas actuales
        self._thumb_timer = QTimer(self)
        self._thumb_timer.setSingleShot(True)
        self._thumb_timer.setInterval(self.THUMB_REFRESH_MS)
        self._thumb_timer.timeout.connect(self._refresh_thumbnails)

        # Sincronizar con el estado inicial
        self.update_layer_list()

    def _sync_thumbnail_canvas(self):
        """Conecta la invalidación al lienzo actual y suelta el anterior."""
        canvas = getattr(self, "canvas", None)
        anterior = getattr(self, "_thumb_canvas_conectado", None)
        if anterior is canvas:
            return
        if anterior is not None:
            signal = getattr(anterior, "contenido_visual_cambiado", None)
            if signal is not None:
                try:
                    signal.disconnect(self._schedule_thumbnail_refresh)
                except (RuntimeError, TypeError):
                    pass
        self._thumb_canvas_conectado = canvas
        signal = getattr(canvas, "contenido_visual_cambiado", None)
        if signal is not None:
            signal.connect(self._schedule_thumbnail_refresh)

    def _schedule_thumbnail_refresh(self):
        """Marca las miniaturas sucias y agrupa cambios visuales en ráfaga."""
        self._thumb_refresh_pending = True
        if self.isVisible() and not self._thumb_timer.isActive():
            self._thumb_timer.start()

    def showEvent(self, event):
        """Aplica al reaparecer cualquier cambio recibido con el panel oculto."""
        super().showEvent(event)
        if (getattr(self, "_thumb_refresh_pending", False)
                and not self._thumb_timer.isActive()):
            self._thumb_timer.start()
        ...
        if anterior is not None:
            ...
            if signal is not None:
                try:
                    ...
                    pass
        self._thumb_canvas_conectado = None
        temporizador = getattr(self, "_thumb_timer", None)
    ...
    def _thumbnail_signatures(self, layer, layer_index):
        """Identidades baratas de las miniaturas principal y de máscara."""
        hl_layer, hl_mask = self._thumb_highlights(layer, layer_index)
        imagen = getattr(layer, "image", None)
        clave_imagen = imagen.cacheKey() if imagen is not None else 0
        tiene_mascara = layer.has_mask()
        efectos = ()
        if not tiene_mascara:
            efectos = tuple(
                efecto.fingerprint()
                for efecto in getattr(layer, "effects", ())
                if getattr(efecto, "activo", False)
            )
        principal = (clave_imagen, tiene_mascara, efectos, hl_layer)
        mascara = getattr(layer, "mask", None)
        firma_mascara = ((mascara.cacheKey(), hl_mask)
                         if mascara is not None else None)
        return principal, firma_mascara

    def _refresh_thumbnails(self):
        """Regenera solo las miniaturas cuya firma visual haya cambiado."""
        ...
        for idx, layer, lbl, mask_lbl in self._thumb_rows:
            ...
            firmas = self._thumbnail_signatures(layer, idx)
            anteriores = self._thumb_signatures.get(layer)
            try:
                if anteriores is None or anteriores[0] != firmas[0]:
                    lbl.setPixmap(self._thumb_pixmap(
                        _imagen_miniatura_capa(layer), hl_layer))
                if (mask_lbl is not None and layer.mask is not None
                        and (anteriores is None or anteriores[1] != firmas[1])):
                    mask_lbl.setPixmap(self._mask_thumb_pixmap(layer.mask, hl_mask))
                self._thumb_signatures[layer] = firmas
            except RuntimeError:
    ...
    def add_mask(self):
        """Añade una máscara blanca a la capa activa y empieza a editarla."""
        if self.canvas is None:
            return
        index = self.canvas.active_layer_index
        if self.canvas.create_mask(from_selection=False):
            self._select_target(index, True)

    def _show_mask_menu(self, layer_index, global_pos):
        """Opciones de la máscara cuya miniatura recibió el clic derecho."""
        if (self.canvas is None
                or not 0 <= layer_index < len(self.canvas.layers)
                or not self.canvas.layers[layer_index].has_mask()):
            return
        self._select_target(layer_index, True)

        from PySide6.QtWidgets import QMenu
        menu = QMenu(self)
        menu.setStyleSheet(theme.menubar_qss())
        acc_invertir = menu.addAction(t("menu.layers.mask.invert"))
        acc_aplicar = menu.addAction(t("menu.layers.mask.apply"))
        menu.addSeparator()
        acc_eliminar = menu.addAction(t("menu.layers.mask.del"))
        for accion, icono in (
                (acc_invertir, ":/icons/invert.png"),
                (acc_aplicar, ":/icons/mask_apply.png"),
                (acc_eliminar, ":/icons/mask_remove.png")):
            if QFile.exists(icono):
                accion.setIcon(theme.icono(icono))

        elegida = menu.exec(global_pos)
        if elegida is acc_invertir:
            self.canvas.invert_mask()
        elif elegida is acc_aplicar:
            self.canvas.apply_mask()
        elif elegida is acc_eliminar:
            self.canvas.remove_mask()
    ...
    def update_layer_list(self):
        ...
        for i in reversed(range(len(self.canvas.layers))):
            ...
            # Si hay máscara, su resultado se enseña en la miniatura separada:
            # la principal conserva los píxeles originales de la capa.
            thumb_label.setPixmap(self._thumb_pixmap(
                _imagen_miniatura_capa(layer), hl_layer))
            ...
            if layer.has_mask():
                ...
                mask_label.setContextMenuPolicy(Qt.CustomContextMenu)
                mask_label.customContextMenuRequested.connect(
                    lambda pos, idx=i, lbl=mask_label:
                        self._show_mask_menu(idx, lbl.mapToGlobal(pos)))

            self._thumb_rows.append((i, layer, thumb_label, mask_label))
            self._thumb_signatures[layer] = self._thumbnail_signatures(layer, i)
    ...
    def _refresh_button_states(self):
        ...
        self.btn_flatten.setEnabled(total > 1)
        layer = self.canvas.get_active_layer_obj()
        self.btn_mask.setEnabled(layer is not None and not layer.has_mask())
