...
from i18n import t
from palette_io import cargar_paleta, guardar_paleta
import theme
...
class _ColorEditorMixin:
    ...
    def _build_custom_swatches(self, layout):
        """Monta muestras propias, el combo de conjuntos (que carga al
        seleccionar) y la botonera compacta de iconos, con el mismo idioma
        visual que la botonera del panel de Capas."""
        ...
        fila.addWidget(self.collection_combo)
        layout.addLayout(fila)

        botonera = QHBoxLayout()
        botonera.setSpacing(2)
        self.btn_add_swatch = self._boton_muestras(
            ":/icons/swatch_add.png", "+",
            t("color.custom_add_tip"), self.add_custom_swatch)
        self.btn_save_collection = self._boton_muestras(
            ":/icons/palette_save.png", "▣",
            t("color.collection.save_tip"), self.save_custom_collection)
        self.btn_import_palette = self._boton_muestras(
            ":/icons/palette_import.png", "⤓",
            t("color.palette.import_tip"), self.import_palette)
        self.btn_export_palette = self._boton_muestras(
            ":/icons/palette_export.png", "⤒",
            t("color.palette.export_tip"), self.export_palette)
        self.btn_delete_collection = self._boton_muestras(
            ":/icons/palette_delete.png", "×",
            t("color.collection.delete_tip"), self.delete_custom_collection)
        self.btn_delete_all = self._boton_muestras(
            ":/icons/swatch_clear.png", "⌫",
            t("color.custom_delete_all_tip"), self.delete_all_custom_swatches)
        for boton in (self.btn_add_swatch, self.btn_save_collection,
                      self.btn_import_palette, self.btn_export_palette,
                      self.btn_delete_collection, self.btn_delete_all):
    ...
    def _boton_muestras(self, icono, alternativo, tooltip, callback):
        """Botón compacto de solo-icono, calcado del _make_btn del panel de
        Capas (borde azul iluminado cuando está habilitado)."""
        boton = QPushButton()
        if QFile.exists(icono):
            boton.setIcon(theme.icono(icono))
        else:
            boton.setText(alternativo)
        boton.setFixedSize(26, 26)
        boton.setCursor(Qt.PointingHandCursor)
        boton.setToolTip(tooltip)
        boton.setStyleSheet(theme.panel_action_button_qss())
    ...
    def _rebuild_custom_swatches(self):
        ...
        self.btn_export_palette.setEnabled(bool(self._custom_colors))
    ...
    def _on_collection_selected(self, *_args):
        """Elegir un conjunto en el combo lo CARGA directamente (con la
        confirmación de siempre si hay muestras que se perderían); si se
        cancela, el combo vuelve a reflejar el estado real de la rejilla."""
        self._update_collection_buttons()
        ...
        if self._custom_colors and not self._confirmar_muestras(t(
            ...
            anterior = self._coleccion_que_coincide()
            self._refresh_collection_combo(
                anterior["name"] if anterior is not None else None)
            return
    ...
    def export_palette(self):
        """Exporta las muestras actuales a un archivo de paleta (.gpl o lista
        hexadecimal AARRGGBB en .txt, que conserva el alfa): así los conjuntos
        pueden viajar entre instalaciones (Windows/Linux) o a otros editores."""
        if not self._custom_colors:
            return
        from PySide6.QtWidgets import QFileDialog
        anterior = getattr(self, "_suppress_block", False)
        # Mismo motivo que en import_palette: el QFileDialog bloquea la ventana
        # y no debe autocerrar este overlay.
        self._suppress_block = True
        try:
            coincidente = self._coleccion_que_coincide()
            nombre = (coincidente["name"] if coincidente is not None
                      else t("color.custom"))
            base = "".join(caracter for caracter in nombre
                           if caracter not in '\\/:*?"<>|').strip() or "Imago"
            sugerida = os.path.join(
                getattr(self._main_window, "last_opened_dir", "") or "",
                base + ".gpl")
            ruta, filtro = QFileDialog.getSaveFileName(
                self._main_window, t("dlg.palette_save"), sugerida,
                t("dlg.filter.palettes_save"))
            if not ruta:
                return
            if os.path.splitext(ruta)[1].lower() not in (".gpl", ".txt"):
                ruta += ".txt" if "*.txt" in filtro else ".gpl"
            self._main_window.last_opened_dir = os.path.dirname(ruta)
            if not guardar_paleta(ruta, self._custom_colors, nombre=nombre):
                from widgets.custom_titlebar import imago_warning
                imago_warning(
                    self._main_window, t("dlg.palette_save"),
                    t("msg.palette.export_error"))
                return
            self._mostrar_estado(
                "status.palette_exported", n=len(self._custom_colors))
        finally:
            self._suppress_block = anterior
